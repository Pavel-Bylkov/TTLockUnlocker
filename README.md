...
- **Общий volume**: для логов и `.env` (оба контейнера используют одни и те же настройки и логи)

## Быстрый старт

1. Скопируйте `.env` из примера ниже и заполните своими данными
2. **Для первого получения chat_id** (если вы не знаете свой chat_id):
   ...
   - Бот автоматически обновит файл `./.env` в текущей папке
...
Оба контейнера используют общий volume `shared_data` для логов и .env:
- `/app/logs` — логи обоих сервисов
- `/app/.env` — общая конфигурация
